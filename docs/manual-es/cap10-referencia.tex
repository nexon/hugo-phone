...
\midrule
\texttt{/etc/asterisk/pjsip.conf} & Define extensiones SIP 101 y 102 (chan\_pjsip) \\
...
\texttt{/opt/hugo-phone/games/dosbox-hugo.conf} & Configuración del emulador (invocada con \texttt{dosbox -conf}) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{plaincode}
pjsip show endpoints        ; estado de extensiones
pjsip show contacts         ; clientes registrados (AOR contacts)
pjsip set logger on/off     ; trazas SIP detalladas
...
\begin{description}
  ...
  \item[DOSBox] Emulador del sistema operativo MS-DOS y del hardware del PC compatible (CPU, VGA, Sound Blaster, etc.).
...
\begin{itemize}
  ...
  \item DOSBox: \texttt{www.dosbox.com}
