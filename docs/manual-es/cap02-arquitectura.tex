...
\begin{tikzpicture}[
  ...
  \node[sw, above=of uinput] (dosbox) {DOSBox\\+ Hugo};

  \node[hw, right=1.4cm of dosbox] (tv) {Televisor\\HDMI};
...
\begin{enumerate}
  ...
  \item El kernel propaga el evento por \texttt{evdev} al subsistema de input, donde es leído por la biblioteca SDL2 que DOSBox utiliza para entrada.
  \item DOSBox traduce el código de tecla a una interrupción 16h de teclado IBM PC, que Hugo lee dentro de su \emph{event loop}.
...
Emulador DOS clásico (paquete \texttt{dosbox} de Debian) que provee CPU x86, VGA y Sound Blaster 16 virtualizados. Se lanza dentro de un servidor X mínimo (\texttt{xinit} + \texttt{openbox}) en modo \emph{fullscreen}. El ISO del juego se monta como unidad CD-ROM mediante \texttt{imgmount} y Hugo arranca automáticamente desde el script \texttt{[autoexec]} del archivo de configuración.
...
DOSBox & \texttt{.bashrc} en TTY1 & Ejecuta Hugo \\
