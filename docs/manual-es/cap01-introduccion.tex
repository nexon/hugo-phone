...
\begin{enumerate}
  ...
  \item Desarrollar un \textbf{puente (bridge)} en Rust que traduzca los tonos DTMF capturados por Asterisk a pulsaciones de teclado inyectadas al emulador DOSBox.
...
\subsection{¿Por qué DOSBox clásico?}

DOSBox (el proyecto original, paquete \texttt{dosbox} en Debian/Raspberry Pi OS) es maduro, está empaquetado en los repos oficiales de Trixie y soporta sin problemas todo lo que Hugo necesita: VGA, Sound Blaster 16, montaje de imágenes ISO con \texttt{imgmount}, y un núcleo \emph{dynamic} con buen rendimiento en ARM. Los \emph{forks} más modernos (DOSBox-Staging, DOSBox-X) aportan shaders OpenGL y KMSDRM más limpio, pero no son estrictamente necesarios para el motor sencillo de Hugo y, al momento de escribir este manual, \texttt{dosbox-staging} no está disponible en los repositorios estándar de la Pi.
