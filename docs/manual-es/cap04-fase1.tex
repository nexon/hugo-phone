...
\begin{itemize}
  ...
  \item Con Asterisk 22 instalado y respondiendo en su CLI (módulo \texttt{chan\_pjsip}).
...
\begin{enumerate}
  ...
  \item Instalación de Asterisk 22, DOSBox, \texttt{build-essential}, \texttt{libudev-dev} y demás dependencias.
  \item Copia de los archivos \texttt{pjsip.conf}, \texttt{extensions.conf} y \texttt{manager.conf} a \texttt{/etc/asterisk/} con permisos correctos.
  ...
  \item Despliegue de la configuración de DOSBox en \texttt{/opt/hugo-phone/games/dosbox-hugo.conf} (se invoca con \texttt{dosbox -conf}).
\end{enumerate}
...
Debe listar las extensiones 101 y 102 (esta última se usará en la siguiente fase) con estado \texttt{Not in use} (sin clientes registrados aún).
