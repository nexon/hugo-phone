...
\midrule
Username & 102 \\
Password & el secreto definido para la extensión 102 en \texttt{pjsip.conf} \\
...
==========================================================================================
 ...
 Endpoint:  102/102                                              Not in use    0 of inf
        Contact:  102/sip:102@192.168.1.20:xxxx                  abcdef0123   Avail        3.000
\end{plaincode}

La extensión 102 debe aparecer con un \emph{Contact} (la IP del dispositivo donde corre Linphone) y estado \texttt{Avail}.
...
\begin{itemize}
  ...
  \begin{shellcode}
sudo asterisk -rvvv
hugo-pbx*CLI> pjsip set logger on
  \end{shellcode}
\end{itemize}
...
\begin{itemize}
  \item Verificar que la extensión 102 esté asignada al contexto \texttt{hugo-game} en \texttt{pjsip.conf}.
...
\begin{itemize}
  \item Problema típico de NAT o de codecs incompatibles. Verificar que \texttt{allow=ulaw} y \texttt{allow=alaw} estén en la plantilla \texttt{endpoint-template} de \texttt{pjsip.conf} y que Linphone también soporte estos codecs en su configuración avanzada.
...
\begin{itemize}
  ...
  \item Confirmar que la plantilla \texttt{endpoint-template} en \texttt{pjsip.conf} tiene la línea \texttt{dtmf\_mode=rfc4733} (equivalente PJSIP de RFC 2833).
\end{itemize}
...
Esto es todo lo que el bridge necesita del lado de Asterisk. La siguiente fase prepara DOSBox y Hugo para que el bridge tenga algo donde inyectar las teclas.
