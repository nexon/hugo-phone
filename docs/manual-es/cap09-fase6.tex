...
\subsection{pjsip.conf: nuevas extensiones}

Agregar extensiones 102, 103, 104 según número de puertos FXS, replicando el bloque de \texttt{endpoint}/\texttt{auth}/\texttt{aor} que usa la 101 (heredando \texttt{endpoint-template}).
...
Para que los jugadores en espera escuchen Hugo, hay que enrutar el audio de DOSBox hacia la sala de conferencia.
...
Crea un dispositivo virtual \texttt{hw:Loopback,0}. DOSBox se configura para sacar audio a ese dispositivo. Asterisk abre el otro extremo del loopback como un canal de entrada y lo \emph{bridge}a a la sala.
