...
\midrule
\texttt{/etc/asterisk/pjsip.conf} & Defines SIP extensions 101 and 102 (chan\_pjsip) \\
...
\texttt{/opt/hugo-phone/games/dosbox-hugo.conf} & Emulator configuration (invoked with \texttt{dosbox -conf}) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{plaincode}
pjsip show endpoints        ; extension states
pjsip show contacts         ; registered clients (AOR contacts)
pjsip set logger on/off     ; detailed SIP traces
...
\begin{description}
  ...
  \item[DOSBox] Emulator of the MS-DOS operating system and the IBM PC compatible hardware (CPU, VGA, Sound Blaster, etc.).
