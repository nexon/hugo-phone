...
\begin{enumerate}
  ...
  \item Develop a \textbf{bridge} in Rust that translates the DTMF tones captured by Asterisk into keystrokes injected into the DOSBox emulator.
...
The Pi 4 is powerful enough to run Asterisk, DOSBox and the bridge at the same time, while remaining a compact, silent, low-power (5--7\,W) \emph{appliance} with direct HDMI output. The 2\,GB model is the cheapest option available and still leaves RAM headroom after all services are loaded.
...
\subsection{Why classic DOSBox?}

DOSBox (the original project, packaged as \texttt{dosbox} on Debian/Raspberry Pi OS) is mature, ships in the official Trixie repos, and supports everything Hugo needs out of the box: VGA, Sound Blaster 16, ISO mounting via \texttt{imgmount}, and a \emph{dynamic} core that performs well on ARM. The more modern \emph{forks} (DOSBox-Staging, DOSBox-X) add OpenGL shaders and a cleaner KMSDRM path, but they are not strictly necessary for Hugo's simple engine and, at the time of writing, \texttt{dosbox-staging} is not available in the standard Raspberry Pi repositories.
