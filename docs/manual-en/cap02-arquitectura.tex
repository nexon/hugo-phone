...
\begin{enumerate}
  ...
  \item The kernel propagates the event through \texttt{evdev} to the input subsystem, where it is read by the SDL2 library that DOSBox uses for input.
  \item DOSBox translates the key code into an IBM PC keyboard interrupt 16h, which Hugo reads inside its \emph{event loop}.
...
Classic DOS emulator (Debian \texttt{dosbox} package) that provides a virtualized x86 CPU, VGA and Sound Blaster 16. It runs inside a minimal X server (\texttt{xinit} + \texttt{openbox}) in \emph{fullscreen} mode. The game ISO is mounted as a CD-ROM drive via \texttt{imgmount} and Hugo launches automatically from the \texttt{[autoexec]} section of the configuration file.
...
DOSBox & \texttt{.bashrc} on TTY1 & Runs Hugo \\
