...
\midrule
Username & 102 \\
Password & the secret defined for extension 102 in \texttt{pjsip.conf} \\
...
Extension 102 must show a \emph{Contact} (the IP of the device running Linphone) with status \texttt{Avail}.
...
\begin{itemize}
  \item Check that extension 102 is assigned to context \texttt{hugo-game} in \texttt{pjsip.conf}.
...
\begin{itemize}
  \item Typical NAT or incompatible-codec problem. Verify that \texttt{allow=ulaw} and \texttt{allow=alaw} are in the \texttt{endpoint-template} of \texttt{pjsip.conf} and that Linphone also supports these codecs in its advanced configuration.
...
\begin{itemize}
  ...
  \item Confirm that the \texttt{endpoint-template} in \texttt{pjsip.conf} has the line \texttt{dtmf\_mode=rfc4733} (PJSIP equivalent of RFC 2833) for extension 102.
\end{itemize}
...
That is everything the bridge needs from the Asterisk side. The next phase prepares DOSBox and Hugo so that the bridge has somewhere to inject the keys.
