...
\subsection{sip.conf: new extensions}

Add extensions 102, 103, 104 according to the number of FXS ports, following the same pattern as the already documented 101.
...
For waiting players to hear Hugo, the audio from DOSBox-Staging must be routed into the conference room.
...
This creates a virtual device \texttt{hw:Loopback,0}. DOSBox-Staging is configured to output audio to that device. Asterisk opens the other end of the loopback as an input channel and \emph{bridges} it into the room.
