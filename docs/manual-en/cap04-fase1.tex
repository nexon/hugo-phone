...
\begin{itemize}
  ...
  \item Running Asterisk 20 and responding on its CLI.
...
\begin{enumerate}
  ...
  \item Installation of Asterisk 20, DOSBox-Staging, \texttt{build-essential}, \texttt{libudev-dev} and other dependencies.
  \item Copies the \texttt{sip.conf}, \texttt{extensions.conf} and \texttt{manager.conf} files to \texttt{/etc/asterisk/} with the correct permissions.
  ...
  \item Deployment of the DOSBox-Staging configuration to \texttt{\~{}/.config/dosbox/}.
\end{enumerate}
...
It must list extensions 101 and 102 (the latter is used in the next phase) with status \texttt{Unmonitored} (no clients registered yet).
