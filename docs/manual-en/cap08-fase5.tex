...
SIP User ID & 101 \\
Authenticate ID & 101 \\
Authenticate Password & the secret for 101 in \texttt{pjsip.conf} \\
Name & Hugo phone 1 \\
...
Extension 101 must appear with the HT801's IP and status \texttt{Avail}. If it says \texttt{Unavail} or the contact is missing, check:

\begin{itemize}
  \item Password match between the HT801 and \texttt{pjsip.conf}.
...
\begin{enumerate}
  \item Confirm DOSBox is showing Hugo on the TV.
...
The dialplan has a 3600-second (1 hour) timeout in \texttt{Read()}. If the call drops earlier, a \emph{session timer} is probably active. Check in \texttt{pjsip.conf} that the \texttt{endpoint-template} has \texttt{timers=no} (PJSIP equivalent of \texttt{session-timers=refuse}).
